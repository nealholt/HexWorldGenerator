...
class Hexagon:
    ...
    def drawExtras(self, surface):
        ...
        """
        if self.road_image is not None:
    ...
    def drawMarkings(self, surface, scale, x_adjust=0, y_adjust=0):
        ...
        if self.marked is not None:
            ...
            n = scale / 100
            radius = int((x_offset / 2) * n)
            center = int(hex_box_adjust * n + self.getX(scale) + radius) + x_adjust, \
                     int(hex_box_adjust * n + self.getY(scale) + radius + height_adjust * scale / 100) + y_adjust
            draw.circle(surface, self.marked, center, radius, 3)

    def collidePoint(self, pos, scale):
        r = self.getRect(scale)
        return r.collidepoint(pos[0], pos[1])
