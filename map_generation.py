...
def markClicked(pos, grid, selected, scale):
    ...
    if selected is not None:
        ...
        neighbors = getPathableNeighbors(selected, grid)
    ...
    if selected is not None and old_selected is not None:
        path = calcShortestPath((selected.row, selected.col), (old_selected.row, old_selected.col), grid)
...
def getPathableNeighbors(cell, grid):
...
def calcShortestPath(coords1, coords2, grid):
    ...
    while len(upcoming) > 0:
        ...
        # cheapest cost
        index = shortPathHelper(upcoming)
        ...
        if (r, c) == (r_end, c_end):
            return path
        neighbor_list = getPathableNeighbors(cell, grid)
...
def calculateCityPaths(grid, city_locations):
    ...
    for i in range(len(city_locations) - 1):
        for j in range(i + 1, len(city_locations)):
            ...
            path = calcShortestPath(city_locations[i], city_locations[j], grid)
...
def DJsPath(starting_cell, ending_cell, grid):
    ...
    while ending_cell not in pathing_cells:
        ...
        for cell in reversed(pathing_cells):
            neighbors = getPathableNeighbors(cell, grid)

            # print(neighbors)

            for n in neighbors:
                new = n[0]
                if new.breadcrumb is None:
...
def resetGrid(frames):
    ...
    start_time = time.time()
    calculateCityPaths(grid, city_locations)
    elapsed = time.time() - start_time
